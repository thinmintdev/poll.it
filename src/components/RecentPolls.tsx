...
const RecentPolls: React.FC = () => {
  const { polls, loading, error, getVoteStats } = useRecentPolls();
  const router = useRouter();
  
  // Auto-scroll setup with better infinite scroll
  const scrollContainerRef = useAutoScroll<HTMLDivElement>([polls.length], {
    enabled: !loading && polls.length > 0,
    scrollSpeed: 0.8,
    pauseOnHover: true
  });
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && scrollContainerRef.current && polls.length > 0) {
      ...
      console.log('Container dimensions:', {
        ...
        infiniteCount: infinitePolls.length
      });
    }
  }, [polls.length, loading, infinitePolls.length]);
      ...
      {loading ? (
        ...
        <div className="space-y-4 py-4 px-4">
          {infinitePolls.map((poll, index) => {
            const { voteStats, totalVotes } = getVoteStats(poll);
